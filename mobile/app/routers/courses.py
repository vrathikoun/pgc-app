...
@router.get("/by-coach/{coach_id}", response_model=List[CourseOut])
def list_courses_by_coach(
    coach_id: int,
    _current: Member = Depends(get_current_member),
    db: Session = Depends(get_db),
):
    """Liste tous les cours assignés à un coach/admin, sans filtre de date."""
    ...
    courses = (
        db.query(Course)
        .filter(Course.coach_id == coach_id)
        .order_by(Course.name, Course.start_time)
        .all()
    )
    return [_enrich(c, db) for c in courses]
